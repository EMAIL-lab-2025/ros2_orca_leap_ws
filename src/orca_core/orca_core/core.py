...
class OrcaHand:
    ...
    def calibrate_manual(self):
        ...
        self.disable_torque()
        print("Motor torque has been disabled. You can now move the joints manually.")

        # 使用本地字典临时存储校准数据，避免在循环中频繁写入文件
        motor_limits_local = self.motor_limits_dict.copy()
        joint_to_motor_ratios_local = self.joint_to_motor_ratios_dict.copy()
        calibrated_joints_to_zero = {}

        # _compute_wrap_offsets_dict 依赖于 motor_limits，先用现有值计算一次
        # 在校准每个关节时，会重置其偏移
        self._compute_wrap_offsets_dict()
        for step in self.calib_sequence:
            for joint in step["joints"].keys():
                motor_id = self.joint_to_motor_map[joint]
                motor_limits_local[motor_id] = [None, None]
                if self._wrap_offsets_dict:
                    self._wrap_offsets_dict[motor_id] = 0.0
        ...
        try:
            ...
            for i, step in enumerate(self.calib_sequence, start=1):
                for joint, _ in step["joints"].items():
                    ...
                    print("-" * 50)
                    print(f"Calibrating Joint: {joint} (Motor ID: {motor_id}) | Progress: {i}/{len(self.calib_sequence)}")
                    
                    # 提示用户将关节弯曲到极限
                    print(f"(1/2) Please move joint '{joint}' to its MAXIMUM FLEXION limit, then press Enter.")
                    input(" > Press Enter to record flexion position...")
                    flex_position = float(self.get_motor_pos(as_dict=True)[motor_id])
                    motor_limits_local[motor_id][1] = flex_position
                    print(f"  -> Flexion limit recorded: {flex_position:.4f} rad")

                    # 提示用户将关节伸展到极限
                    print(f"(2/2) Now, please move joint '{joint}' to its MAXIMUM EXTENSION limit, then press Enter.")
                    input(" > Press Enter to record extension position...")
                    extend_position = float(self.get_motor_pos(as_dict=True)[motor_id])
                    motor_limits_local[motor_id][0] = extend_position
                    print(f"  -> Extension limit recorded: {extend_position:.4f} rad")

                    # 计算并存储传动比
                    delta_motor = abs(motor_limits_local[motor_id][1] - motor_limits_local[motor_id][0])
                    delta_joint = abs(self.joint_roms_dict[joint][1] - self.joint_roms_dict[joint][0])
                    
                    if delta_joint == 0:
                        print(f"Warning: Joint ROM for '{joint}' is zero. Cannot calculate ratio. Skipping.")
                        ratio = 0.0
                    else:
                        ratio = float(delta_motor / delta_joint)

                    joint_to_motor_ratios_local[motor_id] = ratio
                    calibrated_joints_to_zero[joint] = 0.0 # 记录已校准的关节，以便之后归零

                    print(f"Joint '{joint}' calibrated. Motor limits: {motor_limits_local[motor_id]}, Ratio: {ratio:.4f}")

            print("-" * 50)
            print("Manual calibration process completed.")
            
            # 在所有关节都校准完毕后，一次性更新实例属性和YAML文件
            print("Updating configuration files...")
            self.motor_limits_dict = motor_limits_local
            self.joint_to_motor_ratios_dict = joint_to_motor_ratios_local
            
            update_yaml(self.calib_path, 'motor_limits', self.motor_limits_dict)
            update_yaml(self.calib_path, 'joint_to_motor_ratios', self.joint_to_motor_ratios_dict)

            # 重新检查校准状态并更新
            self.calibrated = self.is_calibrated(verbose=True)
            update_yaml(self.calib_path, 'calibrated', self.calibrated)
            print(f"Hand is now {'fully' if self.calibrated else 'partially'} calibrated.")

        except (KeyboardInterrupt, EOFError):
            # 捕获用户中断操作
            print("\n\033[93mCalibration process interrupted by user.\033[0m")
            # 即使中断，也要尝试保存已经校准的数据
            if calibrated_joints_to_zero:
                 print("Saving partially calibrated data...")
                 self.motor_limits_dict = motor_limits_local
                 self.joint_to_motor_ratios_dict = joint_to_motor_ratios_local
                 update_yaml(self.calib_path, 'motor_limits', self.motor_limits_dict)
                 update_yaml(self.calib_path, 'joint_to_motor_ratios', self.joint_to_motor_ratios_dict)
                 self.calibrated = self.is_calibrated(verbose=False)
                 update_yaml(self.calib_path, 'calibrated', self.calibrated)
            return

        finally:
            # 无论校准成功还是中断，都必须执行此部分，以确保机械手处于安全状态
            print("\033[1;33mRestoring hand to operational state...\033[0m")
            # 重新使能扭矩并设置控制模式，为移动关节做准备
            self.enable_torque()
            self.set_control_mode(self.control_mode)
            self.set_max_current(self.max_current)
            time.sleep(0.1) # 短暂等待，确保电机状态稳定

            # 如果有任何关节被校准，则移动到零位
            if calibrated_joints_to_zero:
                print("Move away from the hand. Setting calibrated joints to zero position in:")
                for i in range(3, 0, -1):
                    print(f"  {i}...")
                    time.sleep(1)
                
                # 在移动之前，需要重新计算偏移量，因为motor_limits已经更新
                self._compute_wrap_offsets_dict()
                self.set_joint_pos(calibrated_joints_to_zero, num_steps=50, step_size=0.01)
                print("Hand is now in zero position.")
